...
describe("GraphQL API", () => {
  ...
  beforeAll(() => {
    ...
    userId = "test_graphql_user_" + Date.now() + "_" + rand;
    listingId = "test_graphql_listing_" + Date.now() + "_" + rand;
    const mockUser = { id: userId, name: "GraphQL User", email: "gql_" + Date.now() + "_" + rand + "@test.com", isHost: false };
  ...
  it("fetches listings when authenticated", async () => {
    const query = "query { listings { id title host { name } } }";
    const res = await request(app)
      .post("/graphql")
      .set("Content-Type", "application/json")
  ...
  it("does not return sensitive fields for users", async () => {
    ...
    const badQuery = "query { user(id: \"" + userId + "\") { name email } }";
    const badRes = await request(app)
      .post("/graphql")
      .set("Content-Type", "application/json")
      .set("Authorization", "Bearer " + testUserToken)
      .send({ query: badQuery });
    const badResult = typeof badRes.body === "string" ? JSON.parse(badRes.body) : badRes.body;
