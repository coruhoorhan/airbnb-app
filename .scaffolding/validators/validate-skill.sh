...
# --- Check 3: description field — present, non-empty, 1-600 chars ---
...
if [ -z "$DESC_VALUE" ]; then
    ...
    DESC_LEN=${#DESC_VALUE}
    if [ "$DESC_LEN" -lt 1 ] || [ "$DESC_LEN" -gt 600 ]; then
        ERRORS+=("description length ${DESC_LEN} out of range (must be 1-600 chars)")
    fi
fi
...
# --- Check 4: body must have content after frontmatter (at least 1 line) ---
BODY_LINES=$(awk '/^---$/{if(++s==2) next} s>=2 && NF>0{n++} END{print n+0}' "$FILE")
if [ "$BODY_LINES" -lt 1 ]; then
    ERRORS+=("Body has no content after frontmatter")
fi
