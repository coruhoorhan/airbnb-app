...
export function Navbar({ 
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 dark:bg-[#0D0F12]/95 backdrop-blur-md border-b border-charcoal-border/50 dark:border-white/10 shadow-xs transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-2 sm:gap-4 overflow-hidden">
        ...
        <div 
          ...
          className="hidden md:hidden lg:flex items-center bg-charcoal-bg/70 dark:bg-white/5 hover:bg-charcoal-bg border border-charcoal-border dark:border-white/10 rounded-full py-2 px-4 shadow-xs hover:shadow-md transition-all cursor-pointer text-xs font-semibold divide-x divide-charcoal-border/80 dark:divide-white/10"
         role="button" aria-label="Arama" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.currentTarget.click(); } }}>
