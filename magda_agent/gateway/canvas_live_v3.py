...
class CanvasLiveVisualizerV3:
    """
    Inspired by OpenClaw trends: Implement a CanvasLiveVisualizerV3 in magda_agent/gateway to stream execution state via websockets.
    A real-time canvas visualization component using WebSockets.
    Monitors the agent's internal state and streams execution state.
    """

    def __init__(self, host: str = "0.0.0.0", port: int = 8765):
        """
        Initializes the CanvasLiveVisualizerV3.
    ...
    async def start(self) -> None:
        ...
        """
        logger.info(f"Starting CanvasLiveVisualizerV3 server on {self.host}:{self.port}")
    ...
    async def stop(self) -> None:
        ...
        """
        if self._server:
            logger.info("Stopping CanvasLiveVisualizerV3 server")
            self._server.close()
    ...
    async def broadcast_state(self, state: Dict[str, Any]) -> None:
        """
        Broadcasts the execution state to all connected clients.

        Args:
            state: A dictionary representing the agent's current execution state.
        """
        if not self.clients:
            return
