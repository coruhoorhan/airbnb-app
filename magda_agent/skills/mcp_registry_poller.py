...
class MCPRegistryPoller:
    ...
    def __init__(self, registry: MCPRegistry, mcp_server_url: str, poll_interval: int = 60) -> None:
        """
        Initialize the MCPRegistryPoller.
        ...
        Args:
            ...
            poll_interval (int): Time in seconds between polling attempts. Defaults to 60.
        """
    ...
    async def poll_once(self) -> None:
        """
        Performs a single polling cycle by querying the remote MCP server.
        """
        self.logger.debug(f"Starting poll cycle with MCP server: {self.mcp_server_url}")
    ...
    def start(self) -> None:
        ...
        """
        if self._running:
            self.logger.warning("Polling loop is already running.")
            return

        self._running = True
        loop = asyncio.get_running_loop()
        self._task = loop.create_task(self._sync_loop())
        self.logger.info(f"Started MCPRegistryPoller loop with interval {self.poll_interval}s.")
    ...
    async def stop(self) -> None:
        ...
        if self._task:
            ...
            try:
                ...
                pass
            self._task = None
        self.logger.info("Stopped MCPRegistryPoller loop.")
