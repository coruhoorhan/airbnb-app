...
class ExperienceSkillCreatorV5:
    """
    Creates reusable Python skills dynamically from past execution experiences.
    Inspired by Hermes Agent trend. Extracts skills from raw logs/experience data and outputs
    MCP-compatible JSON-RPC schemas. Natively registers them in procedural memory.
    """
    def __init__(self, llm_client: LLMClient, procedural_memory: Optional[ProceduralMemory] = None) -> None:
        """
        Initializes the ExperienceSkillCreatorV5.
    ...
    async def generate_skill_from_experience(self, problem_description: str, experience_data: str) -> Optional[Dict[str, Any]]:
        """
        Analyzes raw execution experience data and attempts to generate a Python skill.
        Also generates an MCP-compatible JSON-RPC schema for the skill.

        Args:
            problem_description: A description of the problem solved.
            experience_data: The raw string of execution logs or experience data.

        Returns:
            A dictionary containing the generated code and MCP-compatible schema, or None if generation failed.
        """
        prompt = (
            "Analyze the following raw execution logs/experience data of a solved task.\n"
